...
describe("book-session-store", () => {
  ...
  describe("persistBookSession + loadBookSession", () => {
    ...
    it("parses old session files without title field", async () => {
      ...
      const dir = join(tempDir, ".novelix", "sessions");
    ...
    it("读取 legacy JSON 时迁移为 JSONL 并保留 UI thinking", async () => {
      ...
      await expect(readFile(join(tempDir, ".novelix", "sessions", "legacy-1.jsonl"), "utf-8"))
        .resolves
    ...
    it("createAndPersistBookSession 为新 session 写 JSONL 而不是 legacy JSON", async () => {
      ...
      await expect(readFile(join(tempDir, ".novelix", "sessions", "123456-abcdef.jsonl"), "utf-8"))
        .resolves
        .toContain("session_created");
      await expect(readFile(join(tempDir, ".novelix", "sessions", "123456-abcdef.json"), "utf-8"))
        .rejects
        .toThrow();
    });
